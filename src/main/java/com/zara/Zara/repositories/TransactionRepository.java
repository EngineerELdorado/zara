...
public interface TransactionRepository extends PagingAndSortingRepository<PesapayTransaction, Long> {
    ...
    @Query(value = "select * from transaction where created_by_business_id=?1 and not transaction_type ='WITHDRAWAL'  not transaction_type ='PESAPAY_TO_PAYPAL_BUSINESS' ",
    ...
    @Query(value = "select * from transaction where created_by_business_id=?1 and transaction_type='WITHDRAWAL' or transaction)type ='PESAPAY_TO_PAYPAL_BUSINESS'",
