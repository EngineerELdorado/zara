...
public interface TransactionRepository extends PagingAndSortingRepository<PesapayTransaction, Long> {
    ...
    @Query(value = "select * from transaction where lower(status)=?1 or lower(transaction_type)=?1 or lower (transaction_number) like %?1% order by id DESC",nativeQuery = true)
