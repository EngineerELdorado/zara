...
public interface TransactionRepository extends PagingAndSortingRepository<PesapayTransaction, Long> {
    ...
    @Query(value = "select * from transaction where creation_date between ?1 and ?2 order by id desc", nativeQuery = true)
    ...
    @Query(value = "select * from transaction where lower (status) like%?1% or lower (transaction_type) like%?1%  or lower (transaction_number) like %?1% order by id DESC",nativeQuery = true)
